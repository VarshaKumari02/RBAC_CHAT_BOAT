# app/seeds/role_permission_seeder.py
...
def seed_role_permissions(db):
    ...
    role_permissions = [
        ...
        # User role (role_id: 4) gets only View Booking Monitor (1) and Manage PNR (76)
    ...
    for item in role_permissions:
        ...
        exists = (
            ...
            .filter(
                RoleHasPermission.role_id == item["role_id"],
                RoleHasPermission.permission_id == item["permission_id"]
            )
            .first()
        )

        if not exists:
            db.add(RoleHasPermission(**item))
